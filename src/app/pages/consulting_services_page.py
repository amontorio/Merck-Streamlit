...
def single_consultant(id_user, info_user, index):
                        nombre = st_searchbox(
                                ...
                                submit_function= lambda x: (
                                    ...
                                    save_to_session_state("participantes_cs", af.handle_dni_from_name(st.session_state[f"nombre_{id_user}"]), id_user, f"dni_{id_user}"),
                                    save_to_session_state("participantes_cs", af.handle_dni_from_name(st.session_state[f"nombre_{id_user}"]), id_user, f"dni_copy_{id_user}"),
                                    save_to_session_state("participantes_cs", af.handle_centro_from_name(st.session_state[f"nombre_{id_user}"]), id_user, f"centro_trabajo_{id_user}"),
...
if st.sidebar.button("Guardar borrador", use_container_width=True, icon="💾"):
    ...
    archivo_existente = encontrar_formulario_existente(
        user_id, 
        formulario_tipo, 
        datos_ser.get("nombre_necesidades_cs", ""),
        datos_ser.get("start_date_cs", ""),
        datos_ser.get("end_date_cs", "")
    )
