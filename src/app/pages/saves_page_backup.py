...
def init_pagination_state():
    if "page_borr" not in st.session_state:
            filtro_fecha_evento = st.sidebar.text_input("📅 Filtrar por fecha del evento", key='filtro_fecha_evento')t.session_state.page_borr = 0
    ...
    with st.expander(title, expanded=True):
        ...
        for idx, row in page_df.iterrows():
            ...
            cols[0].markdown(
                ...
                f"📌 <strong>Evento:</strong> {row['Nombre del evento']}<br>� <strong>Fecha del evento:</strong> {row['Fecha del evento']}",
...
def aplicar_filtros(df, tipos, fecha_inicio, fecha_fin, nombre_evento, fecha_evento):
    if df.empty:
        return df
    ...
    if fecha_evento:
        df = df[df['Fecha del evento'].str.contains(fecha_evento, case=False, na=False)]
    return df

def reset_filtros():
    for k in ["filtro_tipos", "fecha_rango", "filtro_evento", "filtro_fecha_evento", "page_borr", "page_hist"]:
...
def main():
    ...
    filtro_fecha_evento = st.sidebar.text_input("� Filtrar por fecha del evento", key='filtro_fecha_evento')
